export default {
    slider: {
        items: [
            {
                title: 'New soleo cooper brushed',
                subtitle: 'Kolekcja',
                url: '/',
                type: 'image',
                fileUrls: {
                    'base': 'https://newtrendy.pl/app/uploads/2023/10/new_soleo_740x880dobry_wzor.jpg',
                    'sm': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3-1024x447.jpg',
                    'lg': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3.jpg',
                },
                navTitle: 'New soleo cooper brushed',
            },
            {
                title: 'Na wymiar',
                subtitle: 'Kabiny prysznicowe',
                url: '/',
                type: 'video',
                fileUrls: {
                    'base': 'https://newtrendy.pl/app/uploads/2023/12/Na-Wymiar-1640X716-Winter_maly.mp4',
                },
                navTitle: 'Kabiny na wymiar',
            },
            {
                title: null,
                subtitle: null,
                url: '/',
                type: 'image',
                fileUrls: {
                    'base': 'https://newtrendy.pl/app/uploads/2023/10/new_soleo_740x880dobry_wzor.jpg',
                    'sm': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3-1024x447.jpg',
                    'lg': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3.jpg',
                },
                navTitle: 'Nowy katalog 2023',
            },
            {
                title: 'Active shield 2.0',
                subtitle: 'Powłoka',
                url: '/',
                type: 'video',
                fileUrls: {
                    'base': 'https://newtrendy.pl/app/uploads/2023/10/Powloka-Active-Shield_slider23.mp4',
                },
                navTitle: 'Active shield 2.0',
            },
            {
                title: 'New soleo gunmetal brushed',
                subtitle: 'Kolekcja',
                url: '/',
                type: 'image',
                fileUrls: {
                    'base': 'https://newtrendy.pl/app/uploads/2023/10/new_soleo_740x880dobry_wzor.jpg',
                    'sm': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3-1024x447.jpg',
                    'lg': 'https://newtrendy.pl/app/uploads/2023/10/slider_new_soleo_copper_dobry_wzor3.jpg',
                },
                navTitle: 'New soleo gunmetal brushed',
            },
        ],
    },
    categories: {
        items: [
            {
                label: 'Drzwi wnękowe',
                slug: 'drzwi-wnekowe',
            },
            {
                label: 'Kabiny prysznicowe',
                slug: 'kabiny-prysznicowe',
            },
            {
                label: 'Kabiny walk-in',
                slug: 'kabiny-walk-in',
            },
            {
                label: 'Parawany nawannowe',
                slug: 'parawany-nawannowe',
            },
            {
                label: 'Brodziki i odwodnienia',
                slug: 'brodziki-i-odwodnienia',
            },
            {
                label: 'Nasze kolekcje',
                slug: 'nasze-kolekcje',
            },
            {
                label: 'Kabiny prysznicowe na wymiar',
                slug: 'kabiny-prosnicowe-na-wymiar',
            },
        ],
    },
    productTiles: {
        items: [
            {
                title: 'Avexa',
                subtitle: 'Cooper brushed',
                productSlug: 'avexa-cooper-brushed',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/06/AVEXA_copper_brushed.jpg',
            },
            {
                title: 'Smart',
                subtitle: 'Black',
                productSlug: 'avexa-cooper-brushed',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/09/smart_black_reling2.jpg',
            },
            {
                title: 'New soleo',
                subtitle: 'Gunmetal brushed',
                productSlug: 'avexa-cooper-brushed',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/06/NEW-SOLEO_gunmetal.jpg',
            },
            {
                title: 'New soleo',
                subtitle: 'Light gold',
                productSlug: 'avexa-cooper-brushed',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/07/new_soleo_light_gold.jpg',
            },
        ]
    },
    aboutTiles: {
        items: [
            {
                title: 'architekta',
                subtitle: 'Strefa',
                description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Distinctio eum harum debitis labore omnis provident deserunt maiores velit voluptatibus qui, cumque natus? Officia quidem cupiditate inventore, fuga repellat incidunt impedit?',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/11/Frame-23824-1.webp',
                url: '/',
            },
            {
                title: 'partnera',
                subtitle: 'Strefa',
                description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Distinctio eum harum debitis labore omnis provident deserunt maiores velit voluptatibus qui, cumque natus? Officia quidem cupiditate inventore, fuga repellat incidunt impedit?',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/11/Frame-23826-1.webp',
                url: '/',
            },
            {
                title: 'Serwis',
                description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Distinctio eum harum debitis labore omnis provident deserunt maiores velit voluptatibus qui, cumque natus? Officia quidem cupiditate inventore, fuga repellat incidunt impedit?',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/11/Frame-23826-2.webp',
                url: '/',
            },
            {
                title: 'Inspiracje',
                description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Distinctio eum harum debitis labore omnis provident deserunt maiores velit voluptatibus qui, cumque natus? Officia quidem cupiditate inventore, fuga repellat incidunt impedit?',
                imageUrl: 'https://newtrendy.pl/app/uploads/2023/11/Frame-23827-1.webp',
                url: '/',
            },
        ]
    },
}