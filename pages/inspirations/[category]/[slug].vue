<template>
    <div v-if="!inspirationStore.article.isLoading">
        <SectionsCommonBreadrumbs :breadcrumbs="inspirationStore.article.item.breadcrumbs" />

        <div class="pb-10">
            <p class="uppercase text-[2rem] leading-[2.25rem] mb-10">{{ inspirationStore.article.item.title }}</p>
        </div>
        <div class="mb-20 xl:flex xl:gap-10">
            <div class="pb-10 border-b border-gray-1 mb-10 xl:w-3/4 xl:border-b-0">
                ...
                <div v-html="inspirationStore.article.item.text" class="text-sm [&>p]:mb-4 mt-6 sm:text-base sm:my-10"></div>
            ...
            <div class="xl:w-1/4">
                <SectionsInspirationsArticles :articles="inspirationStore.article.item.similar" class="xl:grid-cols-1" />
            </div>
        </div>
...
onMounted(async () => {
    await inspirationStore.fetchArticle();
})
</script>