<template>
  <MainLayout>
    <LMap ref="map" :zoom="zoom" :center="center" :min-zoom="6.5" @update:zoom="(value) => zoomUpdated(value)"
      :options="mapOptions">
      <LTileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" layer-type="base" name="OpenStreetMap" />
      <MapMarker v-if="!locationsStorage.isLoading && !globalStore.groupedMode" v-for="location in locations"
        :location="location"></MapMarker>
      <MapGroupedMarker v-if="!locationsStorage.isLoading && globalStore.groupedMode" v-for="location in groupedLocations"
        :location="location"></MapGroupedMarker>
    </LMap>
  </MainLayout>
</template>
...
const zoomUpdated = (value: number) => {
  if (zoom.value !== value) globalStore.groupedMode = value <= 6.5;
...
onMounted(async () => {
  ...
  globalStore.groupedMode = zoom.value <= 6.5;
