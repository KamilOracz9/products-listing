<template>
    <div class="border relative">
        ...
        <ul :class="isOpen ? '' : 'hidden'"
            ...
            <li @click="() => onSelect(option.label)" class="px-5 h-[50px] flex items-center border-b last:border-b-0"
...
const { options, placeholder, modelKey } = props;

const model = inject<Ref<string>>(modelKey) ?? ref('');
...
onMounted(() => {
    window.addEventListener('click', (event) => {
        if (!(<HTMLInputElement>event.target)?.classList.contains(`select-${modelKey}`)) isOpen.value = false;
    })
})
</script>