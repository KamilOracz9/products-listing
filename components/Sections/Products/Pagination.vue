    ...
    <section class="flex justify-end mt-10 mb-20 pagination">
        <ul class="flex uppercase text-lg">
            <li v-if="meta.current_page > 1">
                <NuxtLink
                    :to="localePath({ name: 'products', query: { ...router.currentRoute.value.query, page: meta.current_page - 1 } })"
            ...
            <li v-for="link in meta.links.slice(1, -1)"
                ...
                <NuxtLink v-if="link.url"
                    :to="localePath({ name: 'products', query: { ...router.currentRoute.value.query, page: link.label } })">
                    {{ link.label }}</NuxtLink>
...
console.log(meta.value.links.slice(1, -1))
</script>