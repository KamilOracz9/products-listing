            ...
            <SwiperSlide v-for="(slide, index) in productStore.product.item.similarProducts" :key="index">
                <NuxtLink :to="localePath({ name: 'products' }) + `/${slide.slug}`" class="relative group">
                    ...
                    <div class="absolute w-full h-full flex bg-[rgba(0,0,0,.5)] opacity-0 z-10 top-0 left-0 flex-col justify-end p-8 transition-opacity group-hover:opacity-[1]">
                        ...
                        <p class="uppercase text-white mt-1 flex gap-2 items-center lg:mt-4">Zobacz <Arrow :direction="'right'" class="white-filter" :width="12"/></p>
                    </div>
                </NuxtLink>
            </SwiperSlide>
...
const sliderConfig = {
    ...
    'loop': true,
    navigation: {
        nextEl: '.similar-products-slider-arrow-prev',
        prevEl: '.similar-products-slider-arrow-next',
    },
    spaceBetween: 20,
    breakpoints: { 
        450: { slidesPerView: 2 }, 
